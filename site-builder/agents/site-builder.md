...
Follow the **`build-site`** skill exactly (phases 0–5): read the brief → generate a design system with the `design-system` / `ui-ux-pro-max` skills → scaffold Next.js (App Router, TS, Tailwind) → build pages using the `magic` MCP (21st.dev) + Framer Motion, filling real content from the brief → verify `npm run build` and preview render → write `SITE_REPORT.md`.
...
Return: the output project path, the chosen style/palette/fonts, whether Magic was used, how to run it, and any open questions.
