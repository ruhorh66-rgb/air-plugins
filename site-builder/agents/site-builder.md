...
Follow the **`build-site`** skill exactly (phases 0–5): read the brief → generate a design system with the `design-system` / `ui-ux-pro-max` skills → scaffold Next.js (App Router, TS, Tailwind) → build pages with `ui-styling` + shadcn/ui + Framer Motion, filling real content from the brief → verify `npm run build` and preview render → write `SITE_REPORT.md`.
