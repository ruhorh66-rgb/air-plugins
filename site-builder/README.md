...
- **`magic` MCP** (`.mcp.json`) — the [21st.dev MCP](https://21st.dev/mcp) via `mcp-remote` to `https://21st.dev/api/mcp`, generates polished shadcn/Tailwind UI. **OAuth, no API key** — first use opens a 21st.dev sign-in in the browser (21st.dev's recommended keyless setup). Optional — without it the design skills + shadcn still build the site.
