...
Правило контура: у плагина есть вживлённый цикл проверки, и круги считает механизм, а не память. Гейт здесь один и уже машинный — `gates.mjs`.

```text
цель   — собранный сайт проходит гейты качества числами, а не впечатлением
гейт   — node scripts/gates.mjs --url http://127.0.0.1:3000 --out ./
предел — 5 кругов
показ  — один итог: что не проходило, что исправлено, чем подтверждено
```

Запуск через раннер, который считает круги и ведёт журнал попыток:

```
python E:/-7-/air-loop/scripts/loop_run.py ^
  --gate "node scripts/gates.mjs --url http://127.0.0.1:3000 --out ./" ^
  --goal "гейты качества проходят числами" ^
  --limit 5 --protocol .air/loop.jsonl
```

Раннер ничего не чинит и о качестве не судит: вердикт даёт гейт своим кодом возврата, правку между кругами делает исполнитель. На пределе кругов работа останавливается с диагностикой, а не продолжается вслепую — семь ночных прогонов на переформатирование вывода контур уже оплатил.

Отдельный гейт вёрстки, если правится дизайн-система: `python skills/design-system/scripts/html-token-validator.py <файл.html>`.

Канонический текст цикла — скилл `verification-loop` (плагин `air-loop`).
