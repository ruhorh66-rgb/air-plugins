...
func main() {
	...
	mu, _, _ := procCreateMutexW.Call(0, 1, uintptr(unsafe.Pointer(utf16(`Local\air-worker-tray`))))
	if last, _, _ := procGetLastErrorKernel32.Call(); mu != 0 && last == errAlreadyExists {
		return
	}
