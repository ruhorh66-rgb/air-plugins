...
var (
	...
	// GetLastError здесь НЕ ОБЪЯВЛЯЕТСЯ намеренно. Отдельный вызов после другой
	// функции ОС в Go читает чужой код ошибки — см. проверку единственности в
	// main_windows.go. Процедуры нет, чтобы ошибку нельзя было повторить по привычке.
)
