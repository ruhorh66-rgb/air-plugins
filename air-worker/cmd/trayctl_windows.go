...
// trayDisabled — сказал ли владелец машины «нет». Любое непустое значение, кроме «0»,
// считается отказом: человек, поставивший переменную в «false» или «no», имел в виду
// одно и то же, и спорить с ним о написании — худший способ уважить его решение.
func trayDisabled() bool {
	v := strings.TrimSpace(os.Getenv("AIR_WORKER_NO_TRAY"))
	if v == "" {
		v = strings.TrimSpace(userEnvVar("AIR_WORKER_NO_TRAY"))
	}
	return v != "" && v != "0"
}
...
func cmdTray(argv []string) int {
	...
	case *ensure:
		// ВЛАДЕЛЕЦ МАШИНЫ ОБЯЗАН ИМЕТЬ СПОСОБ СКАЗАТЬ «НЕТ», не правя чужой файл.
		//
		// Поставила AIR-ENV-002 14.09.2026, и вопрос правильный: «установил плагин —
		// получил фоновый процесс на каждом старте сессии, и отказаться нечем, кроме
		// правки плагина; мой ЛПР значок не заказывал».
		//
		// Это тот же класс, за который меня уже поправил свой ЛПР по автозапуску:
		// решение о том, что работает на машине, принимает её владелец, а не установщик.
		// Разница лишь в том, что там я протащила запись в реестр, а здесь — процесс.
		//
		// ПОЧЕМУ ПЕРЕМЕННАЯ ОКРУЖЕНИЯ, А НЕ ПОЛЕ В run-config.json. Значок — вещь
		// МАШИНЫ, а не продукта: он один на сеанс и показывает сразу все объявленные
		// продукты. Класть его выключатель в конфигурацию одного продукта значило бы,
		// что второй продукт на той же машине его переопределит, и ответ зависел бы от
		// того, чью сессию открыли первой.
		//
		// Читается и из своего процесса, и из ветви пользователя: процесс, стартовавший
		// раньше объявления переменной, своего окружения не перечитывает — это стоило
		// нам прогона петли 13.09.2026.
		if trayDisabled() {
			if !*quiet {
				say("значок выключен: AIR_WORKER_NO_TRAY")
			}
			return 0
		}
		if lockHeld(trayMutexName) {
