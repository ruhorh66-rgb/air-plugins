...
«Чем меряется» ссылается ТОЛЬКО на проверку судьи из списка ниже (проверка ` + "`имя`" + `)
или на факт реестра (факт ` + "`id`" + `); критерий, которому нечем меряться, судья объявит
«нечем проверить». Проверки судьи продукта: ` + checkList + `
