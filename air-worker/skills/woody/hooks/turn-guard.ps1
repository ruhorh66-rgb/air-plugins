...
# ОТЧЁТ БЕРЁТСЯ ПОСЛЕДНИЙ В ХОДЕ, А НЕ ПЕРВЫЙ. Нашла claude-bd 14.09.2026: ход процитировал
# отчёт другого продукта той же формы («Расстояние : 1 · застой 2 · вердикт THROTTLE» по
# ASW), а свой отчёт шёл ниже. [regex]::Match брал первое совпадение, и страж сверял чужие
# числа с диском своего продукта — ложный отказ при верном диске. Свой отчёт по конвенции
# закрывает ход, поэтому решает последнее совпадение.
function Get-LastMatch([string]$text, [string]$pattern) {
    $all = [regex]::Matches($text, $pattern)
    if ($all.Count -gt 0) { return $all[$all.Count - 1] }
    return [regex]::Match('', '(?!)')
}
...
try {
    ...
    # СЕССИЯ ВЫШЛА ИЗ РАБОТЫ С AIR-WORKER ПО СЛОВУ ЛПР — страж молчит (этап 0.10, шаг 34).
    if (Test-Path -LiteralPath (Join-Path $stateDir "woody-off-$sessionKey.json")) {
        Write-GuardTrace $sessionKey 'пропущено-выключен' 'сессия вышла из работы с air-worker по слову ЛПР'
        exit 0
    }
    ...
    $mDist = Get-LastMatch $msg '(?m)^\s*Расстояние\s*:\s*(\d+|нечем измерить)\s*\S?\s*застой\s*(\d+)\s*\S?\s*вердикт\s*(.+?)\s*$'
    $mTree = Get-LastMatch $msg '(?m)^\s*Дерево\s*:\s*изменено файлов\s*(\d+)'
    ...
    # --- ПРАВИЛО 4: ожидание ЛПР — только по открытому гейту плана (этап 0.10, К9) -------
    #
    # Повод — слова ЛПР сессии Vera 14.09.2026: «без особой надобности вывела на мне запрос…
    # План должен быть файлом, читаемым машиной, поэтому исправляй план, если там что-то не
    # так, и работай». Вопрос к ЛПР законен ровно в двух случаях: в плане есть открытый
    # шаг-гейт, на который ход ссылается, либо судья не может вынести вердикт (код 2). Всё
    # остальное план с целями обязан решать сам; не решает — это дефект плана, и лечится он
    # шагом-гейтом в плане, а не вопросом в чате.
    $mWait = Get-LastMatch $msg '(?m)^\s*От тебя жду\s*:\s*(.+?)\s*$'
    if ($mWait.Success) {
        $wait = $mWait.Groups[1].Value
        $nothing = $wait -match '^(?i)(не жду ничего|ничего|не жду)(?![\p{L}\p{N}])'
        $judge2 = ([string]$drift.judge_code) -eq '2'
        if (-not $nothing -and -not $judge2) {
            $gates = @()
            try { $gates = @((& $exe goals -product $productRoot -json 2>$null | Out-String | ConvertFrom-Json).open_gates) } catch { }
            $named = @($gates | Where-Object { $_ -and ($wait -match ('(?i)гейт\s*' + [regex]::Escape([string]$_) + '(?![\p{L}\p{N}])')) })
            if ($named.Count -eq 0) {
                & $save
                $gateText = 'открытых гейтов в плане нет'
                if ($gates.Count -gt 0) { $gateText = 'открытые гейты плана: ' + ($gates -join ', ') + ' — сошлись на нужный словами «гейт <номер>»' }
                Deny $sessionKey 'ожидание ЛПР без гейта плана' ("ХОД ЖДЁТ ЛПР, А ССЫЛКИ НА ГЕЙТ ПЛАНА НЕТ: «$wait»`n" +
                      "$gateText.`n" +
                      "Решение ЛПР 14.09.2026: вопрос, ответ на который уже есть в целях и плане, не задаётся — план правится, и работа идёт.`n" +
                      "Если ответа в плане действительно нет, внеси шаг-гейт в план ($productRoot) и сошлись на него; иначе работай и напиши «От тебя жду: не жду ничего».")
            }
        }
    }
