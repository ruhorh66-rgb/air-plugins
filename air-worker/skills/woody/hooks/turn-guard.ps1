...
# ВЫВОД — UTF-8, ЯВНО. Windows PowerShell 5.1 кодирует [Console]::Error.WriteLine
# кодовой страницей КОНСОЛИ, а харнесс читает поток как UTF-8: кириллица приезжает
# вопросительными знаками. Поймано живым применением 14.09.2026 — страж отклонил ход,
# и его отказ пришёл как «??? ??? ??????». Страж, чей отказ нельзя прочесть, почти
# бесполезен: причина известна ему и неизвестна тому, кого он остановил.
#
# ЭТО ЗЕРКАЛО УЖЕ ПОЧИНЕННОГО. Ровно та же беда была на ВХОДЕ (stdin читается явным
# UTF-8, см. ниже), и правило тогда записалось только там. Урок, живущий в одном файле,
# не срабатывает в соседнем — здесь он повторился через сутки.
...
try {
    ...
    # ПЕТЛЯ ПО ПРОДУКТУ ИДЁТ В ДРУГОМ ПРОЦЕССЕ — СВЕРЯТЬ ХОД НЕ С ЧЕМ.
    #
    # Найдено 14.09.2026: сессия пустила петлю на ASW в фоне и продолжила работать. Петля
    # правит дерево и переписывает вердикт после каждой итерации, и отчёт, снятый минуту
    # назад, расходится с диском не потому, что ход врёт, а потому, что работа идёт.
    # Страж отказывал бы на каждом ходу, и обход нашёлся один: объявить продуктом сессии
    # другой продукт — то есть обойти стража, чтобы работать честно. Требовать свежий
    # отчёт тоже нельзя: его судья гонял бы тесты по дереву, которое исполнитель правит
    # прямо сейчас. Поэтому ход пропускается, а след называет причину.
    if ($drift.loop_running) {
        Write-GuardTrace $sessionKey 'пропущено-петля' 'по продукту идёт петля: дерево и вердикт меняются под её работой'
        exit 0
    }

    $treeSig = ''
    # ОГРАНИЧЕНИЕ `-- .` ОБЯЗАТЕЛЬНО и обязано совпадать с тем, что делает бинарник:
    # страж сверяет СВОЁ число с числом из отчёта, и стоит им мерить разное — расхождение
    # будет каждый ход, на ровном месте. Без ограничения в моно-репозитории считаются
    # чужие продукты: замер 14.09.2026 дал 10 файлов по репозиторию и 1 по продукту.
    ...
    # ВЕРДИКТ БЕРЁТСЯ ДО КОНЦА СТРОКИ, А НЕ ОДНИМ СЛОВОМ.
    #
    # Прежде стояло `(\S+)`, и оно обрывалось на первом пробеле. Три из четырёх
    # вердиктов двигателя однословные (ALLOW, THROTTLE, ESCALATE) — на них дефект не
    # проявлялся вовсе. Четвёртый, «ЖДЁТ ЛПР», двухсловный: регулярка вытаскивала
    # «ЖДЁТ», сравнивала с полным значением из JSON и давала расхождение ВСЕГДА, при
    # любой вставке, хоть дословной.
    #
    # Цена измерена: страж не давал закрыть ход СОВСЕМ, сколько ни повторяй. Нашла
    # claude-71 14.09.2026, после трёх честных попыток с заново прогнанным отчётом —
    # и не стала подгонять свой вывод под регулярку, что было бы правильно вдвойне:
    # «дословно» значит дословно, а подгонка спрятала бы мой дефект.
    #
    # Урок шире случая: страж сверяет ЧИСЛА с числами, но вердикт — не число, а текст,
    # и разбор текста обязан брать его целиком. Захват «сколько влезет до пробела» —
    # молчаливое допущение о том, чего в договоре не было.
