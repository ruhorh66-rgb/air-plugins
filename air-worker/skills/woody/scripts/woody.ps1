...
# Умолчания бюджетов заданы здесь, а не в примере конфигурации: пример можно не
# дочитать, а умолчание работает всегда. У ЛЕСТНИЦЫ умолчания нет — см. ниже.
# ЛЕСТНИЦА НЕ ИМЕЕТ УМОЛЧАНИЯ. Отсутствие ladder — ОТКАЗ, а не четыре ступени по памяти.
#
# Прежде здесь стояло молчаливое `else { 'script','haiku','sonnet','opus' }`, и это
# скрывало ровно тот случай, ради которого страж хода держал отдельную проверку
# «частичный выход из режима»: снять ladder из конфигурации значило тихо вернуть
# лестницу БЕЗ ступеней усилия (haiku:medium, haiku:max и далее), решённых ЛПР
# 11.09.2026. Петля продолжала работать и печатать ступени, которые человек не
# назначал.
#
# Когда 13.09.2026 надзор за формой сообщения был снят целиком, вместе с ним ушла бы и
# та проверка. Она не восстановлена стражем намеренно: сторож, следящий за ключом в
# чужом файле, слабее отказа в месте применения. Тот же урок, что дал тихий индекс 0 в
# Resolve-LadderTier, — умолчание, подставленное молча, дороже отказа.
#
# @() снаружи — не украшение. Присваивание из if РАЗВОРАЧИВАЕТ массив из одного элемента
# в скаляр, и тогда $ladder[0] вернёт первую БУКВУ строки, а не ступень. Поймано боевым
# прогоном 11.09.2026: в журнале стояло «ступень h» вместо «haiku:low».
$ladder = @(if ($cfg.ladder) { $cfg.ladder } else { @() })
if ($ladder.Count -eq 0) {
    Write-Output 'ОТКАЗ: в run-config.json нет ключа ladder. Это не «возьмём умолчание», а «лестница не назначена».'
    Write-Output 'Лестница — решение ЛПР под конкретный продукт: порядок ступеней и усилий назначает человек, а не код.'
    Write-Output 'Пример: "ladder": ["script", "haiku:medium", "haiku:max", "sonnet:medium", "sonnet:max", "opus:medium", "opus:max"]'
    exit 2
}
