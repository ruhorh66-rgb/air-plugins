...
try {
    ...
    #
    # ЗАПИСЬ — БЕЗ BOM, ЧЕРЕЗ .NET, А НЕ Set-Content -Encoding UTF8. В Windows PowerShell
    # 5.1 этот ключ пишет UTF-8 С BOM (см. то же самое в hooks/mode.ps1, Write-StateJson).
    # PLAN.md читает readPlanGoals построчным разбором без снятия BOM, и первая строка
    # файла с меткой в начале не совпадает с `^\s*\*\*Ц` — блок целей молча не находился
    # бы, план читался бы «без целей», и сравнение упало бы не на том месте, которое
    # проверяет. Найдено этой же правкой: макет ниже сперва писался Set-Content и отказывал
    # «нет ни одной цели» при цели, буквально стоящей первой строкой файла.
    $utf8NoBomFx = New-Object System.Text.UTF8Encoding($false)
    [System.IO.File]::WriteAllText((Join-Path $fx 'run-config.json'),
        '{"judge":{"checks":[],"checklist":"goal/checklist.json","min_facts":7}}', $utf8NoBomFx)
    # БЛОК ЦЕЛЕЙ ОБЯЗАТЕЛЕН И В МАКЕТЕ (этап 0.10, К32, шаг 63): судья и двигатель
    # бинарника с этой правки отказывают кодом 2 на плане без годного блока целей — план
    # без него сравнивал бы не то же самое, что скрипт-эталон видит вовсе не читая. К1
    # привязан к факту `c01`, который есть в реестре при любом вызове Set-Facts ниже
    # (closed везде >= 1): привязка не зависит от того, какой случай проверяется. Скрипты
    # judge.ps1/goal-drift.ps1 блока целей не знают — они его просто не читают, и на
    # равенстве это не сказывается.
    [System.IO.File]::WriteAllText((Join-Path $fx 'PLAN.md'),
        ("**Ц1.** макет даёт равный вердикт у скрипта и у бинарника`n`n" +
         "| Критерий | Цель | Признак достижения | Чем меряется |`n|---|---|---|---|`n" +
         "| К1 | Ц1 | факты реестра посчитаны | факт ``c01`` |`n`n" +
         "| № | Шаг | Ступень | Судья |`n|---|---|---|---|`n" +
         "| ~~1~~ | сделано | ``script`` | ведущая |`n" +
         "| 2 | предстоит | ``script`` | К1: ведущая |`n" +
         "| 3 | выпуск | — | гейт: ЛПР |"), $utf8NoBomFx)
