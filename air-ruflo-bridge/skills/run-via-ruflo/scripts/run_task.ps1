...
try {
    ...
    $delegation = "КАК РАБОТАТЬ (обязательно, это рой, а не одиночная сессия): " +
        ...
        "mcp__claude-flow__hooks_route (либо `ruflo hooks model-route -t <задача>`), " +
        "и спавни агента с посоветованной моделью. Совет НЕ обязателен к исполнению: " +
        "он вероятностный и на малой статистике ошибается — если он противоречит " +
        "существу задачи, бери свою модель и скажи в отчёте, почему. Но СПРОСИТЬ надо " +
        "всегда: маршрутизатор учится только на исходах, которых без вызовов не будет."
