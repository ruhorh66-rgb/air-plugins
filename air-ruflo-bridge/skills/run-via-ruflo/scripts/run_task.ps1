...
try {
    ...
    # ШЕСТЬ РАЗДЕЛОВ ПОСТАНОВКИ — ПРОВЕРКА, А НЕ ПРАВИЛО В ТЕКСТЕ.
    #
    # Стандарт: AIR_OS/04_STANDARDS/AIR_VIBECODING.md § 2а. Правило там записано с
    # 13.08.2026, и одного текста не хватило: задачи без метрик всё равно уходили в рой,
    # а TASK-OBS-0053 закрылся словом «сделано» при десяти несдвинутых порогах. 14.08.2026
    # тем же классом оказались ERR-2026-000236 и -000237: механизм построен, в путь не
    # включён. Поэтому здесь стоит проверка.
    #
    # Проверяется НАЛИЧИЕ разделов, не их качество: машина умеет отличить отсутствующий
    # раздел от присутствующего, но не пустую метрику от осмысленной, и притворяться не
    # будет.
    #
    # Только для задач очереди ($TaskId): ручной прогон с короткой целью — дело человека
    # у консоли, шесть разделов ему предписывать незачем.
    if ($TaskId) {
        $required = [ordered]@{
        ...
        foreach ($name in $required.Keys) {
            if ($Objective -notmatch $required[$name]) { $missing += $name }
        }
        if ($missing.Count -gt 0) {
            $transcript.Add("## ПОСТАНОВКА НЕ ПО СТАНДАРТУ`nНет разделов: " +
                ($missing -join '; ') + ".`nСтандарт — AIR_VIBECODING § 2а, шесть разделов. " +
                "Рою такая задача не подаётся: без метрик прогон закрывается словом " +
                "«сделано», без механизма возвращается прогонами, без настроек гейт " +
                "приходится проходить дважды.")
            Save-Report
            Write-Output ("REFUSED: постановка без разделов — " + ($missing -join '; ') +
                ". Report: $ReportPath")
            exit 11
        }
        $transcript.Add("## Форма постановки`nШесть разделов на месте (AIR_VIBECODING § 2а). " +
            "Проверено наличие, не содержание.")

        # СОГЛАСОВАНИЕ ЛПР — ОТДЕЛЬНО ОТ ФОРМЫ.
        #
        # Найдено 14.08.2026 (ERR-2026-000241): проверка выше ловит отсутствующий раздел, но
        # молчит о том, ВИДЕЛ ли постановку человек. В тот день TASK-OBS-0059 была написана,
        # зарегистрирована, поставлена в очередь и запушена, ни разу не показанная ЛПР, —
        # и ни один механизм этого не заметил. Кнопка зазор не закрывает: она предъявляет
        # сводку dry-run, то есть подтверждает ЗАПУСК, а не согласование текста задачи.
        #
        # Отметку ставит оркестратор по явному слову ЛПР. Значит проверка ловит ЗАБЫВЧИВОСТЬ,
        # а не обман: сессия, решившая проставить дату без спроса, ничем не остановлена. Это
        # тот же предел, что у токена на почтовом сервере, и врать о нём нельзя. Но именно
        # забывчивость и есть основной источник ошибок контура — сегодняшний случай ровно
        # такой.
        if ($Objective -notmatch '(?m)^approved_by_lpr:\s*\d{4}-\d{2}-\d{2}') {
            $transcript.Add("## ПОСТАНОВКА НЕ СОГЛАСОВАНА`nВ шапке нет поля " +
                "``approved_by_lpr: ГГГГ-ММ-ДД``. Шесть разделов на месте, но согласования " +
                "человека нет: рою такая задача не подаётся. Показать ЛПР разделы 1-6, " +
                "получить явное согласие, проставить дату — и только тогда запускать. " +
                "Разбор: ERR-2026-000241.")
            Save-Report
            Write-Output "REFUSED: постановка не согласована ЛПР (нет approved_by_lpr). Report: $ReportPath"
            exit 11
        }
        $transcript.Add("## Согласование`nОтметка ЛПР в шапке постановки есть.")
    }
