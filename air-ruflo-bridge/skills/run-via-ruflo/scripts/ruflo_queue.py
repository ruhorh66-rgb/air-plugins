...
def cmd_push(argv: list[str]) -> int:
    ...
    # РЕЗЕРВ СТРОКИ — СВОЙ ОТДЕЛЬНЫЙ ЗАМОК, НЕ ЗАМОК ИСПОЛНЕНИЯ РОЯ.
    #
    # Переписано 16.08.2026 по указанию ЛПР: «запрос должен прийти в Telegram на
    # следующую задачу, я подтверждаю, а рой после выполнения текущей автоматом её
    # берёт — я для этого уже не нужен». До этой правки push брал ИСПОЛНЯЮЩИЙ замок
    # роя (`hive_claim`) для резерва строки и потому НЕ МОГ выдать кнопку, пока рой
    # занят — хотя нажатие, пришедшее заранее, и так безопасно ждёт своей очереди в
    # `_run_request_impl` (deferred-wait, указание ЛПР 13.08.2026, уже реализовано
    # и не менялось). Занятость роя проверять здесь было ИЗБЫТОЧНО: единственная
    # настоящая причина держать замок вообще — не пустить ДВА push выдать ДВЕ кнопки
    # на одну строку одновременно. Это не требует замка ИСПОЛНЕНИЯ — короткоживущего
    # файла на секунды хватает, и он не блокируется занятостью роя.
    lock_path = os.path.join(REPORTS, "_row_reserve.lock")
    try:
        fd = os.open(lock_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        os.write(fd, f"{os.getpid()} {row['task_id']}".encode("utf-8"))
        os.close(fd)
    except FileExistsError:
        print(f"{row['task_id']}: другой push резервирует строку прямо сейчас — "
              f"повторить через секунду", file=sys.stderr)
        return 4
    try:
