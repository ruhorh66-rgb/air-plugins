...
def _push_next() -> None:
    ...
    ПРОБЕЛ, найденный 16.08.2026 (указание ЛПР — «тишина» после закрытия задачи
    читалась как отказ механизма, хотя механизм ждал ЗАКРЫТИЯ, которое ещё не
    наступило). Раньше эта функция сама молчала до результата: если `push` брал
    время (dry-run) или сеть моргала, между «предыдущая задача закрылась» и
...
def poll_once(chat_id: str) -> int:
    ...
    for upd in resp.get("result", []):
        ...
        if not isinstance(req, dict):
            _answer(cq["id"], "Заявка повреждена — ожидался объект")
            continue
